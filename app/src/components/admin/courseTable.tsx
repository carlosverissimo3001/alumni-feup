import { CourseExtended, CourseExtendedStatusEnum, Faculty } from "@/sdk";
...
export default function CourseTable({
  ...
  const sortedCourses = courses && [...courses].sort((a, b) => {
    ...
    switch (sortKey) {
      ...
      case 'courseType':
        return direction * String(a.courseType).localeCompare(String(b.courseType));
      case 'status':
    ...
    <Card className="shadow-lg border border-gray-200 rounded-lg">
      <CardHeader className="py-4">
        <CardTitle className="flex items-center justify-between text-xl text-[#8C2D19]">
          <span>Courses {courses && courses.length > 0 && `(${courses.length})`}</span> 
          <div className="w-64">
        ...
        {isLoadingCourses ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
                  ...
                  <th className="text-left py-2 px-3 text-[#8C2D19] text-sm">
                    ...
                    >
                      <div className="flex flex-row gap-2 items-center">
                        <span className="text-[#8C2D19] text-sm">Start Year</span>
                        <span className="text-[#948f8e] text-xs">(End Year)</span>
                        <ArrowUpDown size={16} className="ml-1" />
                      </div>
                    </Button>
                  </th>
              ...
              <tbody>
                {sortedCourses?.map((course: CourseExtended) => (
                  <tr
                    key={course.id}
                    className="border-b hover:bg-gray-50 transition-colors duration-200"
                  >
                    <td className="py-2 px-3 text-gray-700 text-sm">{course.name}</td>
                    ...
                    <td className="py-2 px-3 text-gray-700 text-sm">
                      <Badge 
                        variant={course.status === CourseExtendedStatusEnum.Active ? "default" : "secondary"}
                        className={course.status === CourseExtendedStatusEnum.Active 
                          ? "bg-green-100 text-green-800 hover:bg-green-200" 
                          : "bg-gray-100 text-gray-800 hover:bg-gray-200"}
                      >
                        {course.status === CourseExtendedStatusEnum.Active ? "Active" : "Inactive"}
                      </Badge>
                    </td>
                    <td className="py-2 px-3 text-gray-700 text-sm">
                      {course.startYear} {course.endYear ? `(${course.endYear})` : ''}
                    </td>
                  </tr>
                ))}
              </tbody>
