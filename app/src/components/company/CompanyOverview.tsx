...
export default function CompanyOverview({
  ...
  return (
    <Card className="w-[95%] md:w-4/5 lg:w-3/5 mx-auto">
          ...
          <div>
            ...
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center gap-3">
                ...
                {companyType && (
                  <Badge className={companyType.color}>
                    {companyType.label}
                  </Badge>
                )}
              </div>
            ...
            <div className="space-y-2">
              ...
              {data.foundedByAlumni && (
                  <div className="inline-flex items-center gap-2 px-3 py-1.5 my-2 rounded-md bg-primary/10 text-primary border border-primary/20">
                    <GraduationCap className="h-5 w-5" />
                    <span className="text-sm font-medium">Founded by FEUP EI Alumni</span>
                  </div>
                )}
              {data.industry && (
              ...
              {data.headquarters && (
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  ...
                  <span>{data.headquarters.city}, {data.headquarters.country}</span>
                  {data.headquarters.countryCode && (
                    <Image
                      src={`https://flagcdn.com/${data.headquarters.countryCode.toLowerCase()}.svg` || ''}
      ...
      <CardContent className="pt-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:auto-cols-fr md:grid-flow-col gap-x-6 gap-y-3 text-center justify-center">
          {companySize && (
            <div className="flex flex-col gap-2 items-center">
              ...
              <Badge className={`${companySize.color} w-fit`} variant="outline">
                {companySize.label}
              </Badge>
            </div>
          )}
          ...
          {data.founded && (
            <div className="flex flex-col gap-2 items-center">
              ...
              <span className="font-medium text-foreground">{data.founded}</span>
            </div>
          )}
              ...
              <span className="text-sm text-muted-foreground flex items-center gap-1.5">
                ...
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      Average years of professional experience across current employees
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              </span>
              <span className="font-medium text-foreground">{data.averageYOE} years</span>
            </div>
          )}
          ...
          {data.averageYOC && (
            <div className="flex flex-col gap-2 items-center">
              ...
              <span className="font-medium text-foreground">{data.averageYOC}</span>
            </div>
          )}
        </div>
      </CardContent>
